...
# The environment is built at the path it will RUN from, not at the path it
# is built in. ``uv`` writes an absolute interpreter path into the shebang of
# every console script it installs, so an environment created under /build
# and copied to /app leaves every one of them pointing at /build/.venv/bin/
# python -- a path that does not exist in the final image. The scripts are on
# PATH and look fine; running one gives "no such file or directory" naming a
# file that is right there. ``alembic current`` is the one that gets typed by
# hand, and it failed exactly this way; the rest of ``bin/`` was in the same
# state, unnoticed because both CMDs invoke ``python -m``.
