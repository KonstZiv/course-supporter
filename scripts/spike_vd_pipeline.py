"""VD-SPIKE-B v3: Dense Coverage + Hierarchical Memory.

Per-frame Vision LLM analysis with context from previous frames (1-3 images)
and hierarchical text-only memory (instant → scene → course).

Steps (each saves state, resume on restart):
  frames   — extract golden + gap-fill frames, scene segmentation (no API)
  pipeline — Eyes + Memory pipeline (API calls, scene-by-scene)
  eval     — accuracy against ground truth (no API)
  summary  — compare with v1 baseline (no API)

Usage:
    uv run python scripts/spike_vd_pipeline.py                          # all
    uv run python scripts/spike_vd_pipeline.py --step frames            # local only
    uv run python scripts/spike_vd_pipeline.py --step pipeline          # resume API
    uv run python scripts/spike_vd_pipeline.py --step pipeline --scene 5
    uv run python scripts/spike_vd_pipeline.py --step eval
"""
...
TEXT_MODEL = VISION_MODEL  # same model for all calls — consistent testing
...
def step_pipeline(
    ...
    *,
    ...
    vision_pool = GeminiPool(keys, rpm=5) if not dry_run else None
    text_pool = GeminiPool(keys, rpm=15) if not dry_run else None

    frames = frames_state["frames"]
    scenes = frames_state["scenes"]

    # Load existing state
    eyes_state = load_state("eyes") or {"results": {}}
    instant_state = load_state("instant_memory") or {"results": {}}
    scene_mem_state = load_state("scene_memory") or {"results": {}}
    course_mem_state = load_state("course_memory") or {
        "context": "",
        "scenes_processed": -1,
        "history": [],
    }
    ...
    for scene in scenes:
        sid = scene["id"]
        if only_scene is not None and sid != only_scene:
            continue
        ...
        if str(sid) not in instant_state["results"]:
            ...
            save_state("instant_memory", instant_state)
        ...
        if str(sid) not in scene_mem_state["results"]:
            ...
            try:
                ...
                save_state("scene_memory", scene_mem_state)
                log.info(
        ...
        if course_mem_state["scenes_processed"] < sid:
            ...
            save_state("course_memory", course_mem_state)
...
def step_eval(frames_state: dict) -> None:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="VD Pipeline v3: Dense Coverage + Hierarchical Memory"
    )
    parser.add_argument(
        "--step",
    ...
    if step in ("all", "frames"):
        ...
        if not frames_state:
            log.error("Run --step frames first")
            return

    if step in ("all", "pipeline"):
        step_pipeline(frames_state, only_scene=args.scene, dry_run=args.dry_run)

    if step in ("all", "eval", "summary"):
        step_eval(frames_state)
