...
class MethodistAgent:
    ...
    async def _fetch_children_raws(self, course_node_id: Any) -> list[dict[str, Any]]:
        ...
        KD10 leaf→root traversal ensures child Raws are committed when
        the orchestrator visits this node. A child with no Raw row at
        this point is a sequencing violation; the agent does NOT raise
        on it — that surface is the orchestrator's Pass 1 None-handling
        territory — but it drops the child from the prompt input with
        a warning so the LLM never sees a partial / inconsistent
        ...
        for child in children:
            ...
            if child_raw is None:
                logger.warning(
                    "methodist_bottomup_child_raw_missing",
                    parent_node_id=str(course_node_id),
                    child_node_id=str(child.id),
                )
                continue
    ...
    async def _resolve_course_context(self, node: CourseNode) -> tuple[str, str | None]:
        ...
        and language. Walk-depth is bounded the same way as the
        orchestrator's ``_collect_course_nodes`` (defensive 25-level
        cap to surface accidental cycles).
        """
        root = node
        depth = 0
