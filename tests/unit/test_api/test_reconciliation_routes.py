...
class TestReconcilePreview:
    ...
    async def test_202_idempotent_cached(self, client: AsyncClient) -> None:
        """Returns cached job when matching preview exists."""
    ...
    async def test_202_cached_preview_no_job(self, client: AsyncClient) -> None:
        """Falls through to enqueue when preview exists but job is missing."""
        new_job = _make_job()
        editable = _make_editable_node()
        cached_preview = MagicMock()
        cached_preview.id = uuid.uuid4()
        cached_preview.job_id = None

        with (
            patch(_REQUIRE_NODE, new_callable=AsyncMock),
            patch(_EDITABLE_REPO) as mock_repo_cls,
            patch(_ENQUEUE, new_callable=AsyncMock, return_value=new_job),
            patch(
                _COMPUTE_FP,
                new_callable=AsyncMock,
                return_value=("abc123", "def456"),
