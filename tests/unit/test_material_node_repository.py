"""Tests for CourseNodeRepository."""
...
class TestCreate:
    """CourseNodeRepository.create tests."""
    ...
    async def test_create_root_node(self) -> None:
        ...
        assert isinstance(added_node, CourseNode)
...
class TestGetById:
    """CourseNodeRepository.get_by_id tests."""
...
class TestGetRootFor:
    """CourseNodeRepository.get_root_for tests."""
...
class TestUpdateDefaultLanguage:
    """CourseNodeRepository.update handles default_language."""
...
class TestGetTree:
    """CourseNodeRepository.get_subtree tests."""
...
class TestGetTreeIncludeMaterials:
    """CourseNodeRepository.get_subtree with include_documents=True."""

    async def test_include_documents_adds_selectinload(self) -> None:
        """When include_documents=True, materials are loaded for each node."""
        cid = uuid.uuid4()
    ...
    async def test_include_documents_false_by_default(self) -> None:
        """Default is include_documents=False."""
        cid = uuid.uuid4()
        session = AsyncMock()
        exec_result = MagicMock()
        exec_result.scalars.return_value.all.return_value = []
...
class TestGetTreeSetCommittedValue:
    ...
    async def test_uses_set_committed_value_for_orm_objects(self) -> None:
        ...
        repo = CourseNodeRepository(session)
        with patch(
            "course_supporter.storage.course_node_repository.set_committed_value"
        ) as mock_scv:
            await repo.get_subtree(cid)

        # set_committed_value called for: children + parent init (2 nodes)
        # + parent assignment for child node during tree assembly.
        # See course_node_repository.py:get_subtree() lines 189-206.
...
class TestGetTreeOrphanNodes:
    ...
    async def test_orphan_node_treated_as_root(self) -> None:
        """Node with parent_id set but parent not loaded becomes root.
...
class TestMove:
    """CourseNodeRepository.move tests."""
    ...
    async def test_move_to_sibling(self) -> None:
        ...
        # _is_descendant: B.parent_id is None -> not descendant
...
class TestReorder:
    """CourseNodeRepository.reorder tests."""
...
class TestUpdate:
    """CourseNodeRepository.update tests."""
...
class TestNextSiblingOrder:
    """CourseNodeRepository._next_sibling_order tests."""
