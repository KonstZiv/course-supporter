...
# All tests are async because validate_webhook_url is async
# (DNS resolution runs in a thread via anyio.to_thread).
...
class TestValidateWebhookUrl:
    ...
    async def test_http_allowed(self) -> None:
        ...
        with patch(
            ...
            result = await validate_webhook_url("http://example.com/webhook")
        assert result == "http://example.com/webhook"

    async def test_ftp_rejected(self) -> None:
        """Non-HTTP(S) scheme rejected."""
        with pytest.raises(HTTPException) as exc_info:
            await validate_webhook_url("ftp://example.com/file")
    ...
    async def test_no_scheme_rejected(self) -> None:
        ...
        with pytest.raises(HTTPException) as exc_info:
            await validate_webhook_url("example.com/webhook")
        assert exc_info.value.status_code == 422

    async def test_localhost_rejected(self) -> None:
        """localhost hostname rejected."""
        with pytest.raises(HTTPException) as exc_info:
            await validate_webhook_url("https://localhost/webhook")
        assert exc_info.value.status_code == 422
    ...
    async def test_metadata_endpoint_rejected(self) -> None:
        ...
        with pytest.raises(HTTPException) as exc_info:
            await validate_webhook_url("https://metadata.google.internal/v1/")
        assert exc_info.value.status_code == 422
    ...
    async def test_private_ip_127_rejected(self) -> None:
        ...
        with patch(
            ...
            with pytest.raises(HTTPException) as exc_info:
                await validate_webhook_url("https://internal.example.com/hook")
            assert exc_info.value.status_code == 422
            assert "private" in str(exc_info.value.detail)

    async def test_private_ip_10_rejected(self) -> None:
        """10.0.0.0/8 private range rejected."""
        with patch(
            "socket.getaddrinfo",
            return_value=[(2, 1, 6, "", ("10.0.1.5", 0))],
        ):
            with pytest.raises(HTTPException) as exc_info:
                await validate_webhook_url("https://corp.example.com/hook")
            assert exc_info.value.status_code == 422

    async def test_private_ip_172_rejected(self) -> None:
    ...
    async def test_private_ip_192_rejected(self) -> None:
        ...
        with patch(
            ...
            with pytest.raises(HTTPException) as exc_info:
                await validate_webhook_url("https://home.example.com/hook")
            assert exc_info.value.status_code == 422
    ...
    async def test_link_local_rejected(self) -> None:
        ...
        with patch(
            ...
            with pytest.raises(HTTPException) as exc_info:
                await validate_webhook_url("https://cloud.example.com/hook")
            assert exc_info.value.status_code == 422
    ...
    async def test_ipv6_loopback_rejected(self) -> None:
        ...
        with patch(
            ...
            with pytest.raises(HTTPException) as exc_info:
                await validate_webhook_url("https://v6-loopback.example.com/hook")
            assert exc_info.value.status_code == 422
            assert "private" in str(exc_info.value.detail)

    async def test_ipv6_unique_local_rejected(self) -> None:
        ...
        with patch(
            ...
            with pytest.raises(HTTPException) as exc_info:
                await validate_webhook_url("https://v6-private.example.com/hook")
            assert exc_info.value.status_code == 422

    async def test_ipv6_link_local_rejected(self) -> None:
        """IPv6 link-local fe80::/10 rejected."""
        with patch(
            "socket.getaddrinfo",
            return_value=[(10, 1, 6, "", ("fe80::1", 0, 0, 0))],
        ):
            with pytest.raises(HTTPException) as exc_info:
                await validate_webhook_url("https://v6-link.example.com/hook")
            assert exc_info.value.status_code == 422
    ...
    async def test_unresolvable_hostname(self) -> None:
        ...
        with patch(
            ...
            with pytest.raises(HTTPException) as exc_info:
                await validate_webhook_url("https://nonexistent.invalid/hook")
            assert exc_info.value.status_code == 422
    ...
    async def test_no_hostname(self) -> None:
        ...
        with pytest.raises(HTTPException) as exc_info:
            await validate_webhook_url("https:///path")
        assert exc_info.value.status_code == 422
